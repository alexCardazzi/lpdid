...
- ```treat_status```: This should be a vector equal to one for the time period a unit is treated. Never-treated units will have all zeros for this vector. 
...
- **Version 0.5.0** has been updated to:
    - Add in support for FEs.
    - Change the requirement for "rel_time" to "treat_status".
- **Version 0.6.0** has been updated to:
    - Fix a bug that caused `NA` estimates with `composition_correction = TRUE`.
    - Change some argument names to match the `STATA` version of the same function. Additional changes are in the works to make these two packages as similar as possible. Old argument names will continue to work.
